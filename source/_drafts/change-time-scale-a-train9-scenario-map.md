...
A列車で行こう9 で、シナリオマップやプレイ中のゲームの時間倍率や最大車両数の設定を、後から変更する方法をご紹介します。

<!-- more -->

## はじめに

「A列車で行こう9」は、美麗でリアリティのある電車のグラフィックスがとても魅力的です。
しかし、時間経過が速すぎたり、所有できる最大車両数が少なかったりする問題があります。駅のホームから列車が発車しきるまでに3～4時間経過するのは、あまりにもリアリティがなさすぎでしょう。
...
{% gallery change-time-scale-a-train9-scenario-map prepare_step1_2.webp 'ウインドウモードに変更する' 500x608 %}{% endgallery %}
...
{% gallery change-time-scale-a-train9-scenario-map prepare_step2_2.webp 'テンプレートを読み込む' 719x404 %}{% endgallery %}
...
{% gallery change-time-scale-a-train9-scenario-map prepare_step2_4.webp '列車を購入する' 719x404 %}{% endgallery %}
...
{% gallery change-time-scale-a-train9-scenario-map prepare_step2_5.webp '列車を配置する' 719x404 %}{% endgallery %}
...
{% gallery change-time-scale-a-train9-scenario-map temp_step1_1.webp 'ニューゲームの開始' 719x404 %}{% endgallery %}
...
{% gallery change-time-scale-a-train9-scenario-map temp_step1_2.webp 'オプションを変更する' 719x404 %}{% endgallery %}
...
Train メニューの「列車」リストから任意の列車を選んだ状態で、「ダイヤウィザード」ボタンをクリックします。

{% gallery change-time-scale-a-train9-scenario-map temp_step2_1a.webp 'ダイヤウィザードを開始する' 719x404 %}{% endgallery %}

すると、ダイアログボックスが表示されるので、「はい」を選択します。

{% gallery change-time-scale-a-train9-scenario-map temp_step2_1b.webp 'ダイヤウィザードを開始する（ダイアログボックス）' 719x404 %}{% endgallery %}
...
A列車で行こう9 の画面に戻って、「ダイヤウィザード解除」ボタンをクリックします。

{% gallery change-time-scale-a-train9-scenario-map temp_step2_4a.webp 'ダイヤウィザードを終了する' 719x404 %}{% endgallery %}

開始時と同じく、ダイアログボックスが表示されますが、今回は「**いいえ**」を選択します。

{% gallery change-time-scale-a-train9-scenario-map temp_step2_4b.webp 'ダイヤウィザードを終了する（ダイアログボックス）' 719x404 %}{% endgallery %}
...
{% gallery change-time-scale-a-train9-scenario-map change_step2_1.webp 'ダイヤウィザードを開始する' 719x404 %}{% endgallery %}
...
{% gallery change-time-scale-a-train9-scenario-map change_step2_4.webp 'ダイヤウィザードを終了する' 719x404 %}{% endgallery %}

#### (3) 時間・編成数の拡張を変更する

Options メニューから「マップ作成」を選択し、「時間の拡張（実時間比）」項目のサブ項目から、任意の倍率を選んでチェックを入れます。続いて、「車両最大数の拡張」項目のサブ項目から、任意の最大車両数を選んでチェックを入れます。

{% gallery change-time-scale-a-train9-scenario-map change_step3.webp '時間・編成数の拡張を変更する' 719x404 %}{% endgallery %}
