...
血の聖杯は、最大体力増加効果があり、月明りのパワーは、究極技習得またはパッシブスキルレベルアップができるが、ダンジョン内での一時的な効果になる。一方で、ルビーや銀貨は、ダンジョン内のサポートキャラクターの好感度上げに使えるだけでなく、拠点に持って帰ることができ、武器・スキルの強化やステータス強化にも使える。
...
銀貨を消費して、訓練と強化（メイン武器・サブ兵装・スキル）を行える。強化は、定数増加に加え、ある程度のレベルから効果が付与されるので魅力的だが、レベル上昇ごとにコストも倍々で増える。一方、訓練は、倍率増加で微々たる感じだが、レベル上昇ごとに増加するコストも微増で済む。
銀貨は入手しにくい仕様になっているので、強化を初期装備のメイン武器・サブ兵装・スキルのみに絞り、訓練になるべく割り当てていった方が効率的に運用できる。振り分けに失敗したと感じたら、初期化でリセットすれば、銀貨は全て返ってくる。
...
武器の使用にキーボードとマウスカーソルを使いこなす必要があるため、操作性はとても悪い。ゲームパッド使用で改善されるが、狙撃で敵以外のオブジェクトを自動エイムするのは問題だろう。また、キャラクターによって、被弾しやすさが格段に違うことも遊びにくさに繋がっている。
