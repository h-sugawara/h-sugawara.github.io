...
ちなみに、Steam 版でプレイしている場合は、実績の項目と一致するため、アンロック前でも見れる。なお、最後の項目は、おそらく難易度が高いものになっている。
...
{% gallery review-mini-metro mode_selection.webp 'モード選択' 719x404 %}
時間に追われたくないなら、「エンドレス」や「クリエイティブ」で遊ぶのもあり
{% endgallery %}
...
{% gallery review-mini-metro basic_rule1.webp '基本ルール（駅と乗客）' 719x404 %}
駅と乗客（ゲーム内ヘルプから引用）
{% endgallery %}

列車は設定した路線を往復し、停車駅の図形に一致する乗客をすべて降車させ、進行方向にある図形の乗客を定員まで乗車させる。この乗降は自動で行われる。駅に降車客があった場合は、１人につき１ポイント獲得できる。

{% gallery review-mini-metro basic_rule2.webp '基本ルール（列車）' 719x404 %}
列車（ゲーム内ヘルプから引用）
{% endgallery %}
...
駅は周期なく自然に発生し、路線上に増えることもあれば、全く関係ない場所に増えることもある。６人以上乗車待ち客が発生すると混雑状態として扱われ、一定時間が経過すると円グラフでゲームオーバーまでのカウントダウンが始まる。駅を乗換駅にアップグレードすると、客の乗降スピードを速め、乗車待ち人数を増やすことができる。

{% gallery review-mini-metro progress_spec2.webp 'ゲーム進行の仕様（混雑）' 719x404 %}
混雑（ゲーム内ヘルプから引用）
{% endgallery %}
...
ロープライス帯でコストパフォーマンスに優れ、プレイヤースキルを問わない万人向けで、遊び始めたら時間が溶けていく良いゲーム。
...
|      カテゴリ      | 概要            |   点数   |
|:--------------:|---------------|:------:|
| **コストパフォーマンス** | 内容が値段に見合っているか | 18/20点 |
|  **ゲームバランス**   | 遊びやすさや操作性の良さ  | 18/20点 |
|    **世界観**     | 独自性や没入感があるか   | 16/20点 |
|    **ゲーム性**    | 面白さや満足感があるか   | 15/20点 |
|     **音楽**     | BGMのマッチ具合が良いか | 15/20点 |
...
レビューを見て作品が気になったら、Steam (PC)、My Nintendo Store (Nintendo Switch)、App Store (iOS)、Google Play (Android) から購入してプレイしてみてください。
