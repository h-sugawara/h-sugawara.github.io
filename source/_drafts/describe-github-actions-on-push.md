---
title: 一癖も二癖もあるGitHub Actionsのon.pushの解説
...
has_code: true
has_icon: false
og_image: /articles/describe-github-actions-on-push/devops/title.webp
thumbnail: /articles/describe-github-actions-on-push/github_thumbnail.webp
cover:
  image: /articles/describe-github-actions-on-push/devops/cover.webp
  sources:
    small: /articles/describe-github-actions-on-push/devops/cover_small.webp
    medium: /articles/describe-github-actions-on-push/devops/cover_medium.webp
    large: /articles/describe-github-actions-on-push/devops/cover_large.webp
---

GitHub Actions の on.push は、多用するイベントトリガーの一つですが、癖のある仕様で使いこなすのが難しいです。
本記事では、このトリガーの理解を深めるための解説をします。
...
このページは、日本語翻訳されていた記憶があるのですが、今は原文に戻っていて、癖のある仕様でこれを読まないといけないのは、Actions に不慣れな人にとっては苦行ですね。閑話休題。
...
特定のパスを含む**コミット**のプッシュイベントに対して、ワークフローを実行したい時に設定します。
...
公式のドキュメントでは言及されていませんが、コミットのプッシュに関与しない「tags（tags-ignore）」フィルタは、これらと併用しても論理和条件として扱われることからも明白です。

とはいえ、個人的には、「tags（tags-ignore）」フィルタと「paths（paths-ignore）」フィルタが論理積条件にならないことに大いに不満があります。

### 理解度確認テストの解説

これまでの説明で、理解度テストの解答の理由は、お分かりいただけたでしょうか？
...
しかし、下記のようにワークフローを作ると、ブランチとパス、タグとパスの組み合わせで、それぞれで論理積条件にできます。
...
3. 「github.ref_type」の値から、「ブランチとパス」と「タグとパス」のどちらの組み合わせのフィルタか、判定します。
