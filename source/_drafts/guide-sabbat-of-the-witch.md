...
どのような作品か知りたい場合は、{% post_link sabbat-of-the-witch-review 'レビュー記事を投稿しています' %}ので、そちらを是非ご覧ください。
{% endmessage %}

{% message color:warning %}
記事中の「要約フローチャート」、「共通ルート詳細データ」、「個別ルート詳細データ」に記載しているシーンの名前は、公式のものではなく、説明の便宜上名付けた非公式なものですので、取り扱いにご注意ください。
{% endmessage %}
...
そのためには、「喫茶店初回来店時に彼女の制服姿を二度褒めない」、「映画の予定を変更した後にみんなで移動」、「喫茶店に再度来店した時に彼女とじゃれあわない」のいずれかをやっておこう。
