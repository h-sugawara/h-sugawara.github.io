...
インフラ、フロントエンド、バックエンド、SRE、CI/CD などプロダクトに必要な役割は、何でもやる人です（注：バックエンドが本職）。
この記事が投稿される頃は、忙しすぎて死にそうになっているでしょう。誰か私を救ってくれ。

### この記事の存在意義

この記事をひと通り読んだ後に Hexo.js + GitHub Pages のブログを作成できるようになっていること。もしくは、この記事を読みながらブログを作成できること。
...
初回セットアップ時のデフォルトテーマは、landscape です。最低限のことはできますが、見た通りデザインがイケてません。オシャレ感が足りません。そんな人のために、Hexo 公式に[有志が作成したテーマを検索](https://hexo.io/themes/)できるようになっています。
...
記事は、`source/_posts` ディレクトリ直下に、Markdown 形式のファイル（コマンド通りに実行した時は、「2023_12_22_article-name.md」）が生成されます。このファイルを開くと、以下のような内容になっています。
...
`title` は、記事タイトルに使われるので、ここを任意の日本語に変えてください。他にも、テーマによってはカスタムフィールドがあります（例：Icarus テーマなら、`thumbnail` や `cover` 等）ので、お好みで設定してください。
